...
Word.prototype = {
    ...
    estimateCorrectAnswerProbability: function (wordTestResults, now, worstWordProb) {
        var gauss_expecation = statisticUtils.gauss_expecation;
        var gauss_sigma = statisticUtils.gauss_sigma;
        ...
        for (var a in wordTestResults) {
            ...
            var weight = statisticUtils.gaussianDistribution(hours, gauss_expecation, gauss_sigma);
            totalWeight += weight;
            var ans = a.right ? 1 : 0;
        ...
        if (dateUtils.secondsFromTimestamp(now - lastAnswerTimestamp) < 30 && worstWordProb)
